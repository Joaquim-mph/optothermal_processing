"""Chip-72 transfer-curve history: evolution over the full measurement record.

Sibling of plot_chip80_transfer_history.py -- same analysis, chip 72 (hBN,
5 nm top / 31 nm bottom). The CNP is recomputed *in this script* for every dark
IVg (reusing the same parabola algorithm as the derived-metrics pipeline), so
the plots don't depend on the enriched `cnp_voltage` column (which is only
populated once `enrich-history` has been run for the chip). A curve is deemed
**not suitable** if the CNP fit fails OR |V_cnp| > NOT_SUITABLE_ABS_VCNP; for
chip 72 every curve is well-behaved, so that set is typically empty and its
inspection figure is skipped.

Figures (keyed to the sequence index `seq` as the time axis, never wall-clock
time -- except the explicit vs-time view):

  1. chip72_transfer_overlay_seq_gradient.pdf
  ...
  2. chip72_cnp_vs_seq.pdf
       In-script CNP voltage vs sequence index -- how far the neutrality point
       has drifted historically.

  3. chip72_cnp_vs_time.pdf
       Same CNP values vs *elapsed time* (days, first measurement at t=0), so
       idle gaps show as horizontal spacing.

  4. chip72_transfer_not_suitable.pdf   (only if any curve is not suitable)
       The dark IVg curves flagged not suitable, same overlay style.

  5. chip72_rest_effect_dcnp_vs_gap.pdf / chip72_rest_before_after_pairs.pdf
       (only if idle gaps exist) rest-effect analysis: CNP shift across each
       idle gap vs rest duration, and the last-before / first-after pair per gap.

Data: data/03_derived/chip_histories_enriched/Alisson72_history.parquet
...
Run from repo root:
    .venv/bin/python "scripts/chip_history/plot_chip72_transfer_history.py"
"""
...
OUTPUT_DIR = Path("figs/chip72_transfer_history")
...
def material_label(chip: int) -> str:
    """Return e.g. '72 (hBN)' using the bottom-dielectric tag from config."""
...
def load_dark_ivg() -> pl.DataFrame:
    """Dark IVg rows for the chip, ordered by seq, with an in-script `cnp`
    column (recomputed here rather than relying on enriched `cnp_voltage`)."""
    df = (
...
def plot_overlay(
    ...
    ax.text(
        ...
        va="top",
        ha="left",
        zorder=10_000,  # above all curves (their zorder = seq)
...
def plot_cnp_vs_time(
    ...
    """CNP voltage vs elapsed time (days, first measurement at t=0)."""
...
def find_rest_gaps(suitable: pl.DataFrame) -> list[dict]:
    ...
    exceeds GAP_THRESHOLD_DAYS, pairing the last curve before the gap with the
    first after it."""
...
def plot_rest_effect_scatter(
    ...
    """Rest-induced CNP shift vs rest duration. One point per gap."""
...
def plot_rest_before_after_pairs(
    ...
    """One panel per rest gap: last transfer curve before vs first after
    (forward branch), each CNP marked. Panels ordered by increasing rest."""
...
def main() -> None:
    ...
    # Suitable = a CNP was fitted AND it is physically plausible.
    ...
    print(f"  not suitable: {not_suitable.height}")
    if not_suitable.height:
    ...
    plot_overlay(
        suitable,
        OUTPUT_DIR / "chip72_transfer_overlay_seq_gradient.pdf",
        chip_label, seq_norm, config,
    )
    n_figs += 1
    plot_cnp_vs_seq(
        suitable, OUTPUT_DIR / "chip72_cnp_vs_seq.pdf", chip_label, config,
    )
    n_figs += 1
    plot_cnp_vs_time(
        suitable, OUTPUT_DIR / "chip72_cnp_vs_time.pdf", chip_label, config,
    )
    n_figs += 1
    ...
    if not_suitable.height:
        plot_overlay(
            not_suitable,
            OUTPUT_DIR / "chip72_transfer_not_suitable.pdf",
            chip_label, seq_norm, config,
        )
        n_figs += 1
    else:
        print("  (all curves suitable -- skipping not-suitable figure)")
    ...
    if gaps:
        plot_rest_effect_scatter(
            gaps, OUTPUT_DIR / "chip72_rest_effect_dcnp_vs_gap.pdf",
            chip_label, config,
        )
        n_figs += 1
        plot_rest_before_after_pairs(
            gaps, OUTPUT_DIR / "chip72_rest_before_after_pairs.pdf",
            chip_label, config,
        )
        n_figs += 1
    else:
        print("  (no idle gaps -- skipping rest-effect figures)")

    print(f"Wrote {n_figs} figures to {OUTPUT_DIR}/")
