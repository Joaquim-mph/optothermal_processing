...
def plot_per_chip(chip: int, df: pl.DataFrame, outdir: Path) -> None:
    fig, axes = plt.subplots(
        1, 2, figsize=(10, 4.3), constrained_layout=True, sharey=True
    )
    ...
    fig.suptitle(
        f"Encap{chip} — mobility-deficit vs hysteresis ({df.height} looped IVgs)",
        fontsize=11,
    )
...
def plot_cross_chip(
    per_chip: dict[int, pl.DataFrame], responsivity: dict[int, float], outdir: Path
) -> None:
    ...
    if np.isfinite(resps).any():
        ...
        norm = mcolors.Normalize(
            vmin=np.nanmin(log_r),
            vmax=np.nanmax(log_r),
        )
    ...
    fig.suptitle(
        "Across-chip mobility deficit vs hysteresis — colored by photoresponsivity",
        fontsize=11,
    )
...
def main() -> None:
    ...
    print(f"Chips with looped IVgs: {looped['chip_number'].n_unique()}")
    print()
    print(
        f"{'chip':>5} {'n_loop':>7} {'resp':>10} | "
    ...
    for chip in sorted(looped["chip_number"].unique().to_list()):
        ...
        h_s = (
            f"{h_fit['slope']:>+9.3f} {h_fit['r']:>+6.2f} {h_fit['p']:>8.2g}"
            if h_fit
            else f"{'—':>9} {'—':>6} {'—':>8}"
        )
        e_s = (
            f"{e_fit['slope']:>+9.3f} {e_fit['r']:>+6.2f} {e_fit['p']:>8.2g}"
            if e_fit
            else f"{'—':>9} {'—':>6} {'—':>8}"
        )
