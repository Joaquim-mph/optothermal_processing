...
class CalibrationMatcher:
    ...
    def enrich_chip_history(
        ...
        Reads a chip history Parquet file, finds calibrations for all light
        experiments, and writes back an enriched version with new columns:
        ...
        ----------
        history_path : Path
            Path to chip history Parquet file
        force : bool, optional
            If True, overwrite existing calibration associations (default: False)
        stale_threshold_hours : float, optional
        ...
        if "calibration_parquet_path" in history.columns and not force:
            # Already enriched, check how many need updating
            needs_update = history.filter(
                (pl.col(light_col) == True) &
                (pl.col("proc") != "LaserCalibration") &
                (pl.col("calibration_parquet_path").is_null())
            ).height
            ...
            if needs_update == 0:
                return EnrichmentReport(
                    ...
                    matched_stale=0,
                    missing=0,
                    warnings=[f"Already enriched (use --force to overwrite)"],
                    errors=[]
                )
        ...
        backup_path = history_path.with_suffix(".backup.parquet")
        if history_path.exists():
            import shutil
            shutil.copy2(history_path, backup_path)

        # Write enriched history
    ...
    def enrich_all_histories(
        ...
        Finds all *_history.parquet files in the directory and enriches
        them with calibration associations.
        ...
        force : bool, optional
            If True, overwrite existing associations (default: False)
        stale_threshold_hours : float, optional
            Hours beyond which calibration is considered stale (default: 24)
