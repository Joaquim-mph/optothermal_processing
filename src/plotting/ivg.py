...
def plot_ivg_sequence(
    ...
    from src.plotting.shared.transforms import calculate_conductance, calculate_inverse_conductance
    ...
    for row in ivg.iter_rows(named=True):
        ...
        if conductance:
            ...
            if vds is None or vds == 0 or not np.isfinite(vds):
                ...
                logger.warning(f"Skipping seq #{int(row['file_idx'])}: VDS={vds}V (cannot calculate {mode_str})")
                continue
