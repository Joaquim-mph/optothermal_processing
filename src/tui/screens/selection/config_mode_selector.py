...
class ConfigModeSelectorScreen(SelectorScreen):
    """Configuration mode selection screen (Step 3 of wizard)."""

    SCREEN_TITLE = "Configuration Mode"
    STEP_NUMBER = 3
    ...
    # Mode selector CSS (extends SelectorScreen CSS)
    ...
    def compose_header(self) -> ComposeResult:
        """Compose header with title, chip/plot info, and step indicator."""
    ...
    def compose_content(self) -> ComposeResult:
        """Compose configuration mode selector content."""
    ...
    def action_next(self) -> None:
        ...
        # Save mode to session (replaces app.update_config)
        self.app.session.config_mode = mode

        # Navigate to next screen using router
        if mode == "quick":
            # Quick mode: Go directly to experiment selector
            self.app.router.go_to_experiment_selector()
        else:
            # Custom mode: Route to appropriate config screen based on plot type
            if self.plot_type == "ITS":
                ...
                self.app.router.go_to_transconductance_config()
            else:
