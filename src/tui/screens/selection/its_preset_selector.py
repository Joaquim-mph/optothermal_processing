...
class ITSPresetSelectorScreen(SelectorScreen):
    """Screen for selecting ITS plot presets (Step 4)."""

    SCREEN_TITLE = "Choose It Plot Preset"
    STEP_NUMBER = 4
    ...
    def __init__(self, chip_number: int, chip_group: str, plot_type: str = "ITS"):
        super().__init__()
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        if button_id == "back-btn":
            ...
            if preset_name == "custom":
                ...
                self.app.router.go_to_its_config(preset_mode=False)
            else:
                # Show quick config screen for preset (filters only)
                self.app.router.go_to_its_config(preset_mode=True)
